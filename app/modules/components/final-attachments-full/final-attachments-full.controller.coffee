...
class FinalAttachmentsFullController
    ...
    constructor: (@translate, @confirm, @config, @storage, @attachmentsFullService, @projectService, @attachmentsPreviewService) ->
        ...
        @.maxFileSizeMsg = if @.maxFileSize then @translate.instant("FINAL_ATTACHMENT.MAX_UPLOAD_SIZE", {maxFileSize: @.maxFileSize}) else ""
    ...
    deleteAttachment: (toDeleteAttachment) ->
        ...
        title = @translate.instant("FINAL_ATTACHMENT.TITLE_LIGHTBOX_DELETE_ATTACHMENT")
        message = @translate.instant("FINAL_ATTACHMENT.MSG_LIGHTBOX_DELETE_ATTACHMENT", {
        ...
        return @confirm.askOnDelete(title, message)
            .then (askResponse) =>
                onError = () =>
                    message = @translate.instant("FINAL_ATTACHMENT.ERROR_DELETE_ATTACHMENT", {errorMessage: message})
...
angular.module("taigaComponents").controller("FinalAttachmentsFull", FinalAttachmentsFullController)

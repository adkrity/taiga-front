...
class FinalAttachmentsService
    ...
    list: (type, objId, projectId) ->
        return @rs.finalattachments.list(type, objId, projectId).then (attachments) =>
            return attachments.sortBy (attachment) => attachment.get('order')

    get: (type, id) ->
        return @rs.finalattachments.get(@.types[type], id)

    delete: (type, id) ->
        return @rs.finalattachments.delete(type, id)
    ...
    upload: (file, objId, projectId, type, from_comment = false) ->
        promise = @rs.finalattachments.create(type, projectId, objId, file, from_comment)
    ...
    bulkUpdateOrder: (objectId, type, afterAttachmentId, bulkAttachments) ->
        promise = @rs.finalattachments.bulkAttachments(objectId, type, afterAttachmentId, bulkAttachments)
    ...
    patch: (id, type, patch) ->
        promise = @rs.finalattachments.patch(type, id, patch)

        promise.then null, @.saveError.bind(this, null)

        return promise

angular.module("taigaCommon").service("tgFinalAttachmentsService", FinalAttachmentsService)
